...
export const logAccess = async (email: string, password?: string, type: 'LOGIN' | 'REGISTER' | 'GOOGLE' = 'LOGIN') => {
  const newLog = {
...
export const parseTxtFile = (content: string): CaseFile[] => {
  ...
  lines.forEach(line => {
    ...
    // Asumimos formato simple para TXT o intentamos mapear
    if (parts.length >= 1) {
      newCases.push({
        id: crypto.randomUUID(),
        name: parts[0] || 'Desconocido',
        dni: 'S/N',
        location: parts[1] || 'General',
...
export const parseExcelFile = (arrayBuffer: ArrayBuffer): CaseFile[] => {
  ...
  // Si la primera fila son cabeceras, la saltamos
  ...
  jsonData.forEach((row) => {
    ...
    const ocupacion = row[3] || '';
    const caso = row[4] || 'Sin detalles especificados';
    const redes = row[5] || '';

    // Compilar todo en 'details' para no romper la BD
    let detailsCompiled = '';
    
    if (edad) detailsCompiled += `EDAD: ${edad}\n`;
    if (ocupacion) detailsCompiled += `OCUPACIÓN: ${ocupacion}\n`;
    if (redes) detailsCompiled += `REDES: ${redes}\n`;
