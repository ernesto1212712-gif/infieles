...
const SubmitCase: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Compilar detalles para guardar todo junto sin alterar la BD
    let compiledDetails = '';
    if (formData.age) compiledDetails += `EDAD: ${formData.age}\n`;
    if (formData.occupation) compiledDetails += `OCUPACIÓN: ${formData.occupation}\n`;
    if (formData.socials) compiledDetails += `REDES: ${formData.socials}\n`;
    compiledDetails += `\nREPORTE: ${formData.details}`;
  ...
  if (submitted) {
    return (
      <div className="p-8 max-w-2xl mx-auto text-center mt-20 animate-in fade-in slide-in-from-bottom-4">
        ...
        <button 
          onClick={() => { setSubmitted(false); setFormData({name:'', age:'', dni:'', location:'', occupation:'', socials:'', details:''}); }}
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Ocupación / Estudios</label>
              <input
                type="text"
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none dark:text-white"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">DNI (Opcional)</label>
              <input
                type="text"
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none dark:text-white"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Redes Sociales (Opcional)</label>
              <input
                type="text"
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none dark:text-white"
