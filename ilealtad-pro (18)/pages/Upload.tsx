...
const UploadPage: React.FC = () => {
  ...
  const downloadTemplate = () => {
    // Plantilla exacta según la imagen del usuario
    const wsWithHeader = XLSX.utils.aoa_to_sheet([
      ["Nombre", "Edad", "Distrito", "Ocupación", "Caso", "Redes Sociales"],
      ["Juan Perez", "25", "Miraflores", "Estudiante", "Engañó a su pareja...", "@juan_perez"]
    ]);

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, wsWithHeader, "Plantilla_Oficial");
    XLSX.writeFile(wb, "Plantilla_iLealtad.xlsx");
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-8 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
          <div className="bg-green-50 dark:bg-green-900/10 p-6 rounded-lg border border-green-100 dark:border-green-900/30">
            ...
            <p className="text-sm text-green-700 dark:text-green-500 mb-2">El archivo debe tener EXACTAMENTE este orden:</p>
            <ol className="text-xs text-green-800 dark:text-green-400 list-decimal ml-4 space-y-1 font-bold">
              ...
              <li>Ocupación (Columna D)</li>
              <li>Caso / Detalles (Columna E)</li>
              <li>Redes Sociales (Columna F)</li>
            </ol>
          </div>

          <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700">
            <h4 className="font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2 mb-3">
              <FileText size={18} /> Procesamiento Inteligente
            </h4>
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
              El sistema detectará automáticamente los datos de Edad, Ocupación y Redes y los organizará dentro de la ficha del caso.
            </p>
          </div>
        </div>
        ...
        {status === 'error' && (
          <div className="mt-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/50 rounded-lg flex items-start gap-3 animate-in fade-in slide-in-from-bottom-2">
            ...
            <div>
              ...
              <p className="text-red-700 dark:text-red-500 text-sm">El archivo no pudo ser leído o está vacío. Verifique el orden de columnas.</p>
            </div>
          </div>
        )}
      </div>
